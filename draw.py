...
if __name__ == '__main__':
    ...
    with open('ETHBTC-1h-data.csv') as csv_file:
    ...
    test_data = train_data[int(len(train_data) / 1.1):]
    test_answers = train_answers[int(len(train_answers) / 1.1):]

    train_data = train_data[:int(len(train_data) / 1.1)]
    train_answers = train_answers[:int(len(train_answers) / 1.1)]
    ...
    while True:
        ...
        while index_y < len(test_data[graphic]):
            ...
            coord_y = HEIGHT - test_data[graphic][index_y] * 300000 % 1000
